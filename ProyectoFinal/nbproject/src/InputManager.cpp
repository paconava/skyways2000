	...
	case MAINGAME:
		...
		if (paused || startCount || exploding) {
			...
			if (glfwJoystickPresent(GLFW_JOYSTICK_1)) {
				...
				if (axes[3] > axesThreshold) 
					angleAroundPlayer = changeVals(angleAroundPlayer, axes[3] * cameraSpeed, 60.0f, true);
				else if (axes[3] < -axesThreshold)
					angleAroundPlayer = changeVals(angleAroundPlayer, -axes[3] * cameraSpeed, -60.0f, false);
				else
